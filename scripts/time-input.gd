...
func _ready() -> void:
	...
	time_clock.global_position = Vector2(get_viewport_rect().size.x/2 - time_clock.size.x/2 - global_position.x,0)
	manual_time_clock.global_position =  Vector2(get_viewport_rect().size.x/2 - manual_time_clock.size.x/2 - global_position.x,0)
